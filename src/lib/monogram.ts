...
export function monoColorFromUrl(url: string): MonoColor | null {
  ...
  // Non-greedy: with `>` encoded to `%3E` there are no literal `>` stops,
  // so a greedy `[^>]*` would match past the rect into the text fill.
...
//
// Escapes `#`, `<`, `>` only — enough to satisfy the W3C URI spec while
// staying inside Noroff's 300-character avatar URL limit (full
// `encodeURIComponent` would inflate past it). Browsers tolerate the
// unescaped `'`, ` ` chars in `data:image/svg+xml,...`.
